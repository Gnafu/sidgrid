...
/**
 * @author sid&grid
 * GUI to manage the recharge model layer table on postgres by gvSIG
 */

public class DbRchStressManage extends javax.swing.JPanel implements IWindow{
	...
	private JComboBox jComboBoxRecharge;
	...
	private void initGUI() {
		...
		this.add(getLoad());
		this.add(getReset());
		this.add(getComboBoxRech());
		this.add(getJScrollPane1());
		this.setName(PluginServices.getText(this,"Modify_Recharge_Stress_Period"));
		this.setSize(690, 300);
		this.setVisible(true);

		JLabel db = new JLabel(PluginServices.getText(this,"GeoDB"));
		springLayout.putConstraint(SpringLayout.EAST, jComboBoxDB, -6, SpringLayout.WEST, db);
		springLayout.putConstraint(SpringLayout.WEST, db, 210, SpringLayout.WEST, this);
		springLayout.putConstraint(SpringLayout.NORTH, db, 3, SpringLayout.NORTH, getComboBoxRech());
		add(db);

		JLabel lblRech = new JLabel(PluginServices.getText(this,"Recharge"));
		springLayout.putConstraint(SpringLayout.NORTH, lblRech, 26, SpringLayout.NORTH, this);
		springLayout.putConstraint(SpringLayout.WEST, lblRech, 6, SpringLayout.WEST, getAdd());
		springLayout.putConstraint(SpringLayout.EAST, lblRech, 0, SpringLayout.EAST, getAdd());
		add(lblRech);
		add(getBtnCloseTool());
	}
	...
	private JScrollPane getJScrollPane1() {
		if(jScrollPane1 == null) {
			...
			modello.addColumn("ro");
			modello.addColumn("col");

			table = new JTable(modello);
			table.setAutoResizeMode (JTable.AUTO_RESIZE_OFF);
			jScrollPane1 = new JScrollPane(table);
			springLayout.putConstraint(SpringLayout.NORTH, getAdd(), 0, SpringLayout.NORTH, jScrollPane1);
			springLayout.putConstraint(SpringLayout.WEST, getComboBoxRech(), -172, SpringLayout.EAST, jScrollPane1);
			springLayout.putConstraint(SpringLayout.SOUTH, getComboBoxRech(), -24, SpringLayout.NORTH, jScrollPane1);
			springLayout.putConstraint(SpringLayout.EAST, getComboBoxRech(), 0, SpringLayout.EAST, jScrollPane1);
			springLayout.putConstraint(SpringLayout.EAST, jScrollPane1, -6, SpringLayout.WEST, getAdd());
	...
	public WindowInfo getWindowInfo() {
		if (viewInfo == null) {
			...
			viewInfo.setTitle(PluginServices.getText(this,"Modify_Recharge_Stress_Period"));
		}
		return viewInfo;
	}
			...
			show.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent event) {
					...
					try {
					    ...
					    String dbRech = (String)jComboBoxRecharge.getSelectedItem();
					    Connection conn = Utils.getConnectionToDatabase(dbase);
					    
						String sql = "SELECT * FROM "+dbRech+" ORDER BY gid";
//						Connection conn = DriverManager.getConnection("jdbc:postgresql://localhost:5432/"+dbase,"postgres","postgres");
	...
	private JButton getSave() {
		if(apply == null) {
			apply = new JButton();
			springLayout.putConstraint(SpringLayout.NORTH, apply, 93, SpringLayout.NORTH, this);
...
//						Statement stmt=conn.createStatement(ResultSet.CONCUR_UPDATABLE, ResultSet.TYPE_SCROLL_SENSITIVE);
						...
						for (int i = 0; i < table.getRowCount(); i++) {
							...
							{                    			
								...
								PreparedStatement ps = conn.prepareStatement("UPDATE "+dbRech+" SET "+ campo+"=? WHERE gid="+i+"+1");
	...
	private JButton getLoad() {
		if(load == null) {
			load = new JButton();
			springLayout.putConstraint(SpringLayout.NORTH, load, 6, SpringLayout.SOUTH, getSave());
			springLayout.putConstraint(SpringLayout.WEST, load, 0, SpringLayout.WEST, getAdd());
	...
	private JButton getReset() {
		if(reset == null) {
			...
			springLayout.putConstraint(SpringLayout.SOUTH, reset, -39, SpringLayout.SOUTH, this);
		...
		if (btnCloseTool == null) {
			btnCloseTool = new JButton(
					new AbstractAction(PluginServices.getText(this, "Close")) {
						@Override
						public void actionPerformed(ActionEvent evt) {
							PluginServices.getMDIManager().closeWindow(DbRchStressManage.this);
						}
					});
	...
	private JComboBox getComboBoxRech() {
		if(jComboBoxRecharge == null) {
			ComboBoxModel jComboModelTable = 
				new DefaultComboBoxModel();
			jComboBoxRecharge = new JComboBox();
			springLayout.putConstraint(SpringLayout.NORTH, jComboBoxDB, 1, SpringLayout.NORTH, jComboBoxRecharge);
			springLayout.putConstraint(SpringLayout.NORTH, jComboBoxRecharge, 22, SpringLayout.NORTH, this);
			jComboBoxRecharge.setModel(jComboModelTable);
			

		}
		return jComboBoxRecharge;
	}
	
	protected void setTable() {
		final ComboBoxModel jComboBoxFieldModelTable = new DefaultComboBoxModel(Utils.getTableRecharge((String)jComboBoxDB.getSelectedItem()));
		jComboBoxRecharge.setModel(jComboBoxFieldModelTable);
