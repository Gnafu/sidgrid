...
import it.sidgrid.sextante.tools.DrainPolylineToModelGridAlgorithm;
 ...
 * @author sid&grid
 * GUI
 * linear interpolator for drain package. 
 * Extract points from line with interpolated values
 * Request a dbf table with "from" and "to" to interpolate values
 * request DrainPolylineToModelGridAlgorithm SEXTANTE algorithm
 */

public class DrainPolylineToModelInterpolator extends javax.swing.JPanel implements IWindow{
	...
	private JFormattedTextField drnWidth;
	FLayer shape;
	
	public DrainPolylineToModelInterpolator(MapControl mc) {
		super();
		this.mapCtrl = mc;	    
		initGUI();
	...
	private void initGUI() {
		...
		drnWidth = new JFormattedTextField();
		drnWidth.setToolTipText("");
		drnWidth.setText("1");
		drnWidth.setBounds(194, 135, 65, 28);
		add(drnWidth);
		...
		lblxyzPar.setBounds(272, 91, 89, 16);
		add(lblxyzPar);
		
		JLabel lblwidthPar = new JLabel(PluginServices.getText(this,"Width_drain"));
		lblwidthPar.setBounds(271, 141, 99, 16);
		add(lblwidthPar);
		
		JLabel lblNewLabel = new JLabel(PluginServices.getText(this,"Drain_(shp)"));
...
//					FLayers layers = mapCtrl.getMapContext().getLayers();										
//					FLyrVect linegeometry = (FLyrVect)layers.getLayer(Line);
//					final gvVectorLayer layerLine = new gvVectorLayer();
//					layerLine.create(linegeometry);
					...
					PluginServices.getMDIManager().closeWindow(DrainPolylineToModelInterpolator.this);
//					final WaitingPanel test = new WaitingPanel();
					...
					new Thread() {
						...
						public void run() {
							...
							DrainPolylineToModelGridAlgorithm alg =  new DrainPolylineToModelGridAlgorithm();
							ITaskMonitor test_waitingpanel = new DefaultTaskMonitor("DrainPolylineToModelGridAlgorithm", true, null);
							try {								
								...
								int width = Integer.parseInt(drnWidth.getText());
								
								ParametersSet params = alg.getParameters();
								params.getParameter(DrainPolylineToModelGridAlgorithm.LINES).setParameterValue(layerLine);
								params.getParameter(DrainPolylineToModelGridAlgorithm.LENGHT).setParameterValue(jComboVarX.getSelectedIndex());
								params.getParameter(DrainPolylineToModelGridAlgorithm.WIDTH).setParameterValue(width);
								params.getParameter(DrainPolylineToModelGridAlgorithm.TABLE).setParameterValue(tableStress);
								params.getParameter(DrainPolylineToModelGridAlgorithm.DISTANCE).setParameterValue(cell);
								params.getParameter(DrainPolylineToModelGridAlgorithm.LAYER).setParameterValue(layer);
								params.getParameter(DrainPolylineToModelGridAlgorithm.XYZ).setParameterValue(xyz);
								OutputObjectsSet outputs = alg.getOutputObjects();
								Output out = outputs.getOutput(DrainPolylineToModelGridAlgorithm.RESULT);	
...
//								alg.setAnalysisExtent(m_AnalysisExtent);	
								...
								String layerName = "drn_point_tmp";
...
private JButton getBtnCloseTool() {
	...
	public WindowInfo getWindowInfo() {
		if (viewInfo == null) {
			...
			viewInfo.setTitle(PluginServices.getText(this,"Drain_line_interpolator"));
			viewInfo.setHeight(236);
