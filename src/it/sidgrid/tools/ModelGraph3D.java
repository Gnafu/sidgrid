...
public class ModelGraph3D extends javax.swing.JPanel implements IWindow{
	...
	public ModelGraph3D(MapControl mc) {
		this.mapCtrl = mc;
		setLayout(null);
		ComboBoxModel ModelLayerModel = new DefaultComboBoxModel(Utils.getModelsNames());
	...
	public WindowInfo getWindowInfo() {
		if (viewInfo == null) {
			...
			viewInfo.setTitle(PluginServices.getText(this, "View_3D_model"));
	...
	public class SwingAction extends AbstractAction {
		...
		public void actionPerformed(ActionEvent e) {							    
		    try {
				...
				FLyrVect griglia = (FLyrVect)layers.getLayer("model_layer_1");
...
//			    colCont.initGreyWedge();
			    ...
			    data_ref.setData(createDataset(layerGriglia, jCOL, iROW, "TOP", limit, zmin, zmax) );			    
			    ...
			    PluginServices.getMDIManager().closeWindow(ModelGraph3D.this);
	...
	public FlatField createDataset(gvVectorLayer layerGriglia, int NCOLS, int NROWS, String field, boolean limit, double zmin, double zmax)
			throws IteratorException, VisADException, RemoteException {
		int elevation = layerGriglia.getFieldIndexByName(field);
	    
	    double[] quota = new double[NROWS*NCOLS];
	  	    
		IFeatureIterator iterGeo1 = layerGriglia.iterator();
		int i = layerGriglia.getShapesCount();
		IFeature featureGeo1;
		IRecord recordGeo1;
		Double h;
		if(limit){
			if(i>0){
				featureGeo1 = iterGeo1.next();
				recordGeo1 = featureGeo1.getRecord();			 
				h = (Double) recordGeo1.getValue(elevation);
				quota[0] = (h>zmax)?zmax:(h<zmin)?zmin:h;
			}
			for (int j = 1; j < i; j++)
			{							
				 featureGeo1 = iterGeo1.next();
				 recordGeo1 = featureGeo1.getRecord();			 
				 h = (Double) recordGeo1.getValue(elevation);
				 quota[j] = (h>zmax)?quota[j-1]:(h<zmin)?quota[j-1]:h;
			}
		}else
		{
			for (int j = 0; j < i; j++)
