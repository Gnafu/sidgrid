...
public class VectorToModelGrid extends javax.swing.JPanel implements IWindow {
	...
	public VectorToModelGrid(MapControl mc) {
		super();
		this.mapCtrl = mc;	    
		initGUI();
	...
	private void initGUI() {
		...
		this.setName(PluginServices.getText(this,"Vector_to_Model_Grid"));
	...
	public WindowInfo getWindowInfo() {
		if (viewInfo == null) {
			...
			viewInfo.setTitle(PluginServices.getText(this,"From_Vector"));
	...
	private JLabel getJLabelParams() {
		if(jLabelParams == null) {
			...
			jLabelParams.setText(PluginServices.getText(this,"Vector_Layer"));
		...
		if(abstractActionCalculate == null) {
			abstractActionCalculate = new AbstractAction(PluginServices.getText(this, "Run"), null) {
				...
				public void actionPerformed(ActionEvent evt) {
					PluginServices.getMDIManager().closeWindow(VectorToModelGrid.this);
					...
					new Thread() {
						...
						public void run() {
							...
							try {
								...
								IFeatureIterator iter2 = layerVar.iterator();
...
//								rs = stmt.executeQuery(sql);
								...
								String fieldGridX = jComboModelX.getSelectedItem().toString();
								String fieldGridY = jComboModelY.getSelectedItem().toString();
								String fieldGridZ = jComboModelZ.getSelectedItem().toString();

								while (iter1.hasNext()) {
									final IFeature feature = iter1.next();
									IRecord record = feature.getRecord();
									int gid = (Integer) record.getValue(0);
									final Geometry g = feature.getGeometry();
									iter2 = layerVar.iterator();
									test.setValue(i++);
									while (iter2.hasNext()) {
										final IFeature feature2 = iter2.next();
										final Geometry g2 = feature2.getGeometry();
										if (g2.intersects(g.getCentroid())) {	

											if (field2x != -1 && fieldGridX.contentEquals("unselected") != true)
											{
												double valueX = (Double) feature2.getRecord().getValue(field2x);
//												if (jComboModelX.getSelectedItem().toString().contains("KX")){
//													String sqlx = "update "+selectedLayerGriglia+" set \"KX\" =" +new Double(valueX)+" where gid="+gid;
//													stmt.executeUpdate(sqlx);
//												}
//												else{
													String sqlx = "update "+selectedLayerGriglia+" set "+"\""+jComboModelX.getSelectedItem().toString()+"\""+" =" +new Double(valueX)+" where gid="+gid;
													stmt.executeUpdate(sqlx);
//												}
					
//												rs.updateDouble(fieldGridX, new Double(valueX));
												
											}
											if (field2y != -1 && fieldGridY.contentEquals("unselected") != true)
											{
												double valueY = (Double) feature2.getRecord().getValue(field2y);
//												if (jComboModelY.getSelectedItem().toString().contains("KY")){
//													String sqly = "update "+selectedLayerGriglia+" set \"KY\" = "+new Double(valueY)+" where gid="+gid;
//													stmt.executeUpdate(sqly);
//												}
//												else{
													String sqly = "update "+selectedLayerGriglia+" set "+"\""+jComboModelY.getSelectedItem().toString()+"\""+" =" +new Double(valueY)+" where gid="+gid;
													stmt.executeUpdate(sqly);
//												}
												
//												rs.updateDouble(fieldGridY, new Double(valueY));
											}
											if (field2z != -1 && fieldGridZ.contentEquals("unselected") != true)
											{
												double valueZ = (Double) feature2.getRecord().getValue(field2z);
//												if(jComboModelZ.getSelectedItem().toString().contains("KZ")){
//													String sqlz = "update "+selectedLayerGriglia+" set \"KZ\" = "+new Double(valueZ)+" where gid="+gid;
//													stmt.executeUpdate(sqlz);
//												}
//												else{
													String sqlz = "update "+selectedLayerGriglia+" set "+"\""+jComboModelZ.getSelectedItem().toString()+"\""+" =" +new Double(valueZ)+" where gid="+gid;
													stmt.executeUpdate(sqlz);
//												}
												
//												rs.updateDouble(fieldGridZ, new Double(valueZ));
											}					    			
//											rs.next();
//											rs.updateRow();
